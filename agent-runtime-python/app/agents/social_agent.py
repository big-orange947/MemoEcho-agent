...
class SocialAgent(BaseAgent):
    ...
    def _append_delegated_task_instruction(
        ...
        return (
            ...
            "每轮最多推进一个必要问题，保持真实 QQ 私聊的简短表达。默认只写一条自然消息；只有确实有两个彼此独立、都需要立即表达的信息时，"
            "才用一个换行分成两条，换行代表连续发送两个气泡。不要为了凑气泡拆开同一句。陈述通常不要句号或感叹号，真实问句保留一个问号。"
            "不得使用 Markdown、括号动作、舞台说明、客服确认话术或解释性旁白。任务是否完成由后置工作流依据对方明确回复判断，你不要自行宣布内部任务完成。"
        )
