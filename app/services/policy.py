...
# 策略: 是否要求"拿不准先请示"(HITL)
# ---------------------------------------------------------------------------
def resolve_hitl(conversation: dict[str, Any]) -> bool:
    """该会话是否要求"拿不准的事先请示号主"。

    默认开启(新会话 require_human_confirmation=1):
    用自然语言从主控台下命令时,不需要额外配置就是 HITL 模式 ——
    agent 遇到超出授权、或依赖现实背景的事会先回来问,而不是自己拍板。

    显式关掉之后,agent 在目标范围内自主决定(少来回问),但**底线不变**:
    不越界承诺、不编造、不替号主做重大决定 —— 那条写死在提示词里,
    与这个开关无关(关掉只是"少问",不是"胆子变大")。
    """
    return bool(_as_bool_int(conversation.get("require_human_confirmation")))
